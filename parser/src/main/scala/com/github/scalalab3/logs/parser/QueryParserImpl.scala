...
object QueryParserImpl extends QueryParser with RegexParsers {
  ...
  private lazy val mapQ: Parser[Query] = (
        level ~ "="  ~ Level.names.`'case-insensitive'`  ^^ { (_, _, s2) => Eq(level, find(s2, Level).toString) }
      | level ~ "!=" ~ Level.names.`'case-insensitive'`  ^^ { (_, _, s2) => Neq(level, find(s2, Level).toString) }

      | stringKeys.`case-sensitive` ~ "=" ~ stringVal         ^^ { (s1, _, s2) => Eq(s1, s2) }
      | stringKeys.`case-sensitive` ~ "!=" ~ stringVal        ^^ { (s1, _, s2) => Neq(s1, s2) }
      | stringKeys.`case-sensitive` ~ "contains" ~ stringVal  ^^ { (s1, _, s2) => Contains(s1, s2) }
  ...
  private lazy val periodQ = "(\\d+)".r ~ TimeUnit.names.`case-insensitive` ^^ { (n, unit) => Period(n.toLong, find(unit, TimeUnit)) }

  implicit class SeqToParser(seq: Seq[String]) {
    def `case-sensitive`: Parser[String] = s"(${seq.mkString("|")})".r
    def `case-insensitive`: Parser[String] = s"(?i)(${seq.mkString("|")})".r
    def `'case-insensitive'`: Parser[String] = s"(?i)(${seq.mkString("'", "'|'", "'")})".r ^^ { case stringVal(s) => s }
  }
